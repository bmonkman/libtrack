...
export class InitialSchema1700000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "user" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying NOT NULL, CONSTRAINT "PK_user_id" PRIMARY KEY ("id"))`
    );

    // Add foreign keys
    await queryRunner.query(
      `ALTER TABLE "passkey_credential" ADD CONSTRAINT "FK_passkey_credential_user_id" FOREIGN KEY ("user_id") REFERENCES "user"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`
    );
    await queryRunner.query(
